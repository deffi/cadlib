...
from cadlib.util.vector import Vector, X, Y, Z, origin
...
class TestFrustum(TestCase):
    def test_construction(self):
        ...
        with self.assertRaises(TypeError): Frustum(0, Y, 1, 0)
        with self.assertRaises(TypeError): Frustum(X, 0, 1, 0)
    ...
    def test_direction_length(self):
        self.assertEqual(Frustum.direction_length(X        , 5, 1, 2), Frustum(origin, X*5, 1, 2))
        self.assertEqual(Frustum.direction_length((1, 0, 0), 5, 1, 2), Frustum(origin, X*5, 1, 2))

        # Zero direction
        with self.assertRaises(ValueError): Frustum.direction_length(origin, 5, 1, 2)
    ...
    def test_to_scad(self):
        ...
        self.assertScadObject(Frustum(origin, 5*Z, 1, 1), "cylinder", [5.0], [('r', 1)])

        # Cone along Z axis
        self.assertScadObject(Frustum(origin, 5*Z, 1, 0), "cylinder", [5.0], [('r1', 1), ('r2', 0)])

        # Frustum along Z axis
        self.assertScadObject(Frustum(origin, 5*Z, 2, 1), "cylinder", [5.0], [('r1', 2), ('r2', 1)])

        # Cylinder along other axes (X, Y, -X, -Y, -Z)
        cylinder_scad = ScadObject("cylinder", [5], [('r', 1)], None)
        self.assertEqual(Frustum(origin, 5*X, 1, 1).to_scad(),
            ScadObject("rotate", [], [('a', 90.0), ('v', [0.0, 1.0, 0.0])], [
                cylinder_scad]))
        self.assertEqual(Frustum(origin, 5*Y, 1, 1).to_scad(),
        ...
        self.assertEqual(Frustum(origin, -5*Y, 1, 1).to_scad(),
            ScadObject("rotate", [], [('a', 90.0), ('v', [1.0, 0.0, 0.0])], [
                cylinder_scad]))
        self.assertEqual(Frustum(origin, -5*Z, 1, 1).to_scad(),
    ...
    def test_repr(self):
        self.assertRepr(Frustum(origin, 5*X, 1, 2), "Frustum(Vector(0, 0, 0), Vector(5, 0, 0), 1, 2)")
