...
class TestTransformGenerators(TestCase):
    ...
    def test_rotate_invalid(self):
        # Nothing at all
        with self.assertRaises(TypeError): rotate()

        # Canonical forms - multiple specifications
        with self.assertRaises(TypeError): rotate(axis = X, angle = 45, frm = X)
        with self.assertRaises(TypeError): rotate(axis = X, angle = 45, to = X)
        with self.assertRaises(TypeError): rotate(axis = X, angle = 45, xyz = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(axis = X, angle = 45, ypr = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(frm = X, to = Y, axis = X)
        with self.assertRaises(TypeError): rotate(frm = X, to = Y, angle = 45)
        with self.assertRaises(TypeError): rotate(frm = X, to = Y, xyz = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(frm = X, to = Y, ypr = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(xyz = [1, 2, 3], ypr = [1, 2, 3])

        # Canonical forms - incomplete specification
        with self.assertRaises(TypeError): rotate(axis = X)    # Missing angle
        with self.assertRaises(TypeError): rotate(angle = 45)  # Missing axis
        with self.assertRaises(TypeError): rotate(frm = X)     # Missing to
        with self.assertRaises(TypeError): rotate(to = Y)      # Missing from
        ...
        with self.assertRaises(TypeError): rotate(X, 45, axis = X)
        with self.assertRaises(TypeError): rotate(X, 45, angle = 45)
        with self.assertRaises(TypeError): rotate(X, Y, frm  = X)
        with self.assertRaises(TypeError): rotate(X, Y, to = Y)

        # Convenience forms, multiple specifications
        with self.assertRaises(TypeError): rotate(X, 45, frm = X)
        with self.assertRaises(TypeError): rotate(X, 45, to = X)
        with self.assertRaises(TypeError): rotate(X, 45, xyz = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(X, 45, ypr = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(X, Y, axis = X)
        with self.assertRaises(TypeError): rotate(X, Y, angle = 45)
        with self.assertRaises(TypeError): rotate(X, Y, xyz = [1, 2, 3])
        with self.assertRaises(TypeError): rotate(X, Y, ypr = [1, 2, 3])

        # Convenience forms - incomplete specification
        with self.assertRaises(TypeError): rotate(X)
    ...
    def test_scale_invalid(self):
        # Nothing at all
        with self.assertRaises(TypeError): scale()

        # Canonical forms - multiple specifications
        with self.assertRaises(TypeError): scale(xyz = [1, 1, 1], factor = 2)
        with self.assertRaises(TypeError): scale(xyz = [1, 1, 1], axis = X)

        # Canonical forms - incomplete specification
        with self.assertRaises(TypeError): scale(axis = X)  # Missing factor
        ...
        # Convenience forms, duplicate specification
        with self.assertRaises(TypeError): scale(X, 2, axis = X)
        with self.assertRaises(TypeError): scale(X,    xyz = X)

        # Convenience forms, multiple specifications
        with self.assertRaises(TypeError): scale(X, 2, xyz = X)
        with self.assertRaises(TypeError): scale(2,    xyz = X)
